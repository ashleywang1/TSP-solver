...
#endif
#endif
                                                                                                                                                            /* ------------------- size_t and alignment properties -------------------- *//* The byte and bit size of a size_t */
#define SIZE_T_SIZE         (sizeof(size_t))
#define SIZE_T_BITSIZE      (sizeof(size_t) << 3)
                                                                            /* Some constants coerced to size_t *//* Annoying but necessary to avoid errors on some platforms */
...
static void init_malloc_global_mutex()
{
    for (;;) {
        ...
        if (stat == 0 &&
            interlockedcompareexchange(&malloc_global_mutex_status, -1,
                                       0) == 0) {
        ...
        if (tbase == CMFAIL) {  /* Cope with partial failure */
            if (br != CMFAIL) { /* Try to use/extend the space we did get */
                if (asize < HALF_MAX_SIZE_T &&
                    ...
                    size_t esize =
                        granularity_align(nb + SYS_ALLOC_PADDING - asize);
...
static void **ialloc(mstate m,
                     size_t n_elements,
                     size_t * sizes, int opts, void *chunks[])
{
    /*
    ...
    if (marray == 0) {
        ...
        set_size_and_pinuse_of_inuse_chunk(m, array_chunk,
                                           array_chunk_size);
...
void *dlmalloc(size_t bytes)
{
    ...
    if (!PREACTION(gm)) {
        ...
        if (bytes <= MAX_SMALL_REQUEST) {
            ...
            binmap_t smallbits;
            nb = (bytes <
                  MIN_REQUEST) ? MIN_CHUNK_SIZE : pad_request(bytes);
            idx = small_index(nb);
