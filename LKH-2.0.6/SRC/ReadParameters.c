 ...
 *
 * MEMETIC = { YES | NO }
 * Specifies whether the memetic algorithm is used.
 * Default: NO
 *
 * MOVE_TYPE = <integer>
 ...
 * KARP, K-MEANS, ROHE, MOORE or SIERPINSKI. DELAUNAY specifies that the 
 * Delaunay partitioning scheme is used, KARP that Karp's partitioning scheme
 * is used, K-MEANS that a partitioning scheme based on K-means clustering is 
 * used, ROHE that Rohe's random rectange partitining scheme is used, and 
